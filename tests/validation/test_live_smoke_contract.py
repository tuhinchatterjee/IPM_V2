...
def test_the_estimate_is_derived_from_the_catalogue():
    ...
    # Eight role pings now. Five came from §29 asking quick verification to
    # cover the complex planner as well as the routine one; the sixth and
    # seventh are R2's split of the old single analyst role into the cheap
    # investigator that gathers and the deep analyst that judges; the eighth is
    # Playbook's author. Every one of them is called in production, so every one
    ...
    assert lv.ESTIMATED_CALLS[lv.QUICK] == 16
...
def test_the_call_total_is_the_sum_of_the_cases(offline_quick):
    ...
    assert report.live_calls_made == 16
