...
/**
 * RETIRED FROM ROUTING (implementation plan Section 16 — "one canonical
 * Early Warning nav item"). This file lives in a Next.js private folder
 * (the leading underscore excludes it from the App Router), so it is no
 * longer reachable at any URL: `/early-warning/signals` now redirects to
 * `/early-warning` (see the stub at ../signals/page.tsx), which surfaces
 * this screen's real 123-signal Version 2 catalogue directly.
 *
 * The component below is kept, not deleted, because its underlying
 * building blocks (signal-view.ts's ordering/labelling rules, the
 * CreditStory renderer, the borrower drill-down pattern) are exactly the
 * "retained, refactored" pieces named in the migration plan — a future
 * pass re-sources this screen's data from early_warning_v2 rather than
 * the 34-signal taxonomy it reads today.
 */
