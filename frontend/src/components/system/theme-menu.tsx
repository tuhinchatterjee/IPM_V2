 ...
 * reload: the theme is a `data-theme` attribute on <html> and all four palettes
  ...
  return (
    <div ref={container} className={cn("relative", className)}>
      ...
      >
        ...
        <span className="hidden md:inline">{ready ? active.name : "Theme"}</span>
        {/* The active palette, shown as three squares — recognisable at a glance
            even when the label is hidden on a narrow window. */}
        <span className="flex overflow-hidden rounded-[3px] border border-border" aria-hidden>
          {active.swatch.map((colour, i) => (
            <span key={i} className="size-2.5" style={{ backgroundColor: colour }} />
          ))}
        </span>
      </button>
      ...
      {open && (
        ...
        >
          <p className="border-b border-border px-3 py-2 text-[10px] font-medium uppercase tracking-[0.12em] text-text-muted">
            Theme
          </p>
          <ul className="p-1">
            {THEMES.map((option) => {
                  ...
                  >
                    ...
                    >
                      {option.swatch.map((colour, i) => (
                        <span key={i} className="size-3.5" style={{ backgroundColor: colour }} />
                      ))}
                    </span>
                    ...
                    <ModeIcon className="size-3 shrink-0 text-text-muted" aria-hidden />
