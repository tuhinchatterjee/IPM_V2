...
import { api, type EwsSelectionView, type WhatIfThreadView }
  from "@/lib/api";
import { useAsync } from "@/lib/hooks";
import { cn } from "@/lib/utils";

import { ColourKey, CutChart, Panel, count, money } from "@/app/early-warning/whatif/charts";
import { ResultBlock } from "@/app/early-warning/whatif/result";
...
/**
 * One What-If thread, wherever it was opened.
 *
 * §8 of the demo completion contract: the standalone page, a guided card and
 * an Early Warning export are three DOORS onto one workspace. They were three
 * implementations — one had method selection, a measured waterfall and a
 * workbook; the other ran a scenario inline under the guided cards and showed
 * the reader `pd_relative`. Different entry context is legitimate. A reduced
 * feature set is not.
 *
 * Give it a `threadId` to reopen a persisted conversation. Give it a
 * `selectionId` and it opens (or reopens) the thread for that cohort, which
 * is what keeps the old Early Warning deep links working.
 */
export function WhatIfThread({ threadId = "", selectionId = "",
                               openedFrom = "early_warning_export",
                               title = "" }: {
  threadId?: string; selectionId?: string;
  openedFrom?: string; title?: string;
}) {
  const [thread, setThread] = React.useState<WhatIfThreadView | null>(null);
  const [opening, setOpening] = React.useState(true);
  const [openError, setOpenError] = React.useState("");

  // A thread id reopens; a selection id opens one over that cohort. Either
  // way what comes back is the same object, so everything below this is the
  // same code for all three doors.
  React.useEffect(() => {
    let alive = true;
    setOpening(true);
    (async () => {
      try {
        const got = threadId
          ? await api.whatifThread(threadId)
          : await api.whatifThreadOpen({ selection_id: selectionId,
                                         opened_from: openedFrom, title });
        if (alive) { setThread(got); setOpenError(""); }
      } catch (failed) {
        if (alive) setOpenError(String(failed));
      } finally {
        if (alive) setOpening(false);
      }
    })();
    return () => { alive = false; };
  }, [openedFrom, selectionId, threadId, title]);

  const resolvedSelection = thread?.selection_id ?? selectionId;
  const load = React.useCallback(
    () => api.ewsSelection(resolvedSelection),
    [resolvedSelection]);
  const { data, loading, error } = useAsync<EwsSelectionView>(
    load, [load], { enabled: Boolean(resolvedSelection) });
  ...
  // The conversation comes back from the database, not from React state.
  // Holding it in state meant closing the tab lost four scenarios while the
  // selection survived, so reopening a thread in front of a committee showed
  // a cohort and an empty conversation.
  React.useEffect(() => {
    if (!thread || hydrated.current === thread.thread_id) return;
    hydrated.current = thread.thread_id;
    const restored: Turn[] = [];
    for (const held of thread.turns ?? []) {
      const kind = String(held.kind ?? "");
      if (kind === "said") {
        restored.push({ id: nextId.current++, kind: "said",
                        text: String(held.text ?? "") });
      } else if (kind === "result" && held.result) {
        restored.push({ id: nextId.current++, kind: "result",
                        said: String(held.said ?? ""),
                        result: held.result as Loose });
      } else if (kind === "clarification") {
        restored.push({ id: nextId.current++, kind: "asked",
                        question: String(held.question ?? ""),
                        readAs: (held.read_as as string[]) ?? [] });
      }
    }
    if (restored.length) setTurns(restored);
    if (thread.method) setMethod(thread.method);
  }, [thread]);
  ...
  const run = React.useCallback(async (said: string) => {
    ...
    setBusy(true);
    try {
      // One shape for both calls. The thread route can answer with
      // `needs_method`, which the selection route cannot, and a union of the
      // two narrows to the poorer of them.
      const got = (thread
        ? await api.whatifThreadAsk(thread.thread_id,
                                    { said: sentence, method })
        : await api.ewsSelectionRun({
            selection_id: resolvedSelection, said: sentence, method })
      ) as Awaited<ReturnType<typeof api.whatifThreadAsk>>;
      if (got.needs_clarification) {
        add({ kind: "asked", question: String(got.question ?? ""),
              readAs: (got.read_as as string[]) ?? [] });
      } else if (got.needs_method) {
        // Not an error. The thread has read the sentence and is asking which
        // model to run it through, with the interpreted shock already on
        // screen — §8.3 forbids running Delta and calling it the reader's
        // choice.
        add({ kind: "asked",
              question: `${String(got.because ?? "")} Interpreted as: `
                        + `${String(got.described ?? sentence)}.`,
              readAs: (got.read_as as string[]) ?? [] });
      } else {
  ...
  }, [add, busy, method, resolvedSelection]);

  const choose = React.useCallback(async (which: string) => {
    setMethod(which);
    if (thread) {
      try {
        await api.whatifThreadMethod(thread.thread_id, { method: which });
      } catch {
        // The choice still applies to this session; it simply will not be
        // remembered on reopen, which is better than refusing the click.
      }
    }
  }, [thread]);
  ...
  const download = React.useCallback(async (said: string) => {
    ...
    try {
      const { blob, filename } = await api.ewsSelectionWorkbook({
        selection_id: resolvedSelection, said, method });
  ...
  }, [add, method, resolvedSelection]);

  const openCustomers = React.useCallback(async () => {
    setShowCustomers((was) => !was);
    if (customers.length) return;
    try {
      const got = await api.ewsSelectionCustomers(resolvedSelection, 200);
      setCustomers(got.customers);
    } catch (failed) {
      add({ kind: "note", tone: "warn", text: String(failed) });
    }
  }, [add, customers.length, resolvedSelection]);

  if (openError) {
    return <EmptyState title="This What-If could not be opened"
                       description={openError} />;
  }
  if (opening || (loading && !data)) {
  ...
  created_by: selection.created_by,
      ...
      <Card className="sticky bottom-3 z-10 border-border/80 bg-surface/95 p-4
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {METHODS.map((one) => (
            <button key={one.key} type="button" onClick={() => void choose(one.key)}
                    title={one.hint}
                    className={cn(
