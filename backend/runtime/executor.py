...
def execute(plan: AnalyticalPlan | dict[str, Any], *,
    ...
    Two governed boundaries pass through here, and both are forwarded to
    `validate()` unchanged.

    `scope` says which governed boundary the caller is inside. It defaults to
    GENERAL, so the scorecard validation domains are refused unless a caller
    has said, in its own code, that it is one of the two things allowed to
    read them.

    `domain_lock` is how a thread scoped to one product surface (e.g. Early
    Warning) refuses a plan that reads outside it. This is the only place in
    the live compute path where either refusal can be enforced, since every
    caller reaches this function — which is also why neither may be dropped
    ...
    report = validate(plan, limits=limits, scope=scope,
                      domain_lock=domain_lock).raise_if_bad()
