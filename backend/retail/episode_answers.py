...
#: What Alpha narrows by at each step, since it carries no evidence-state
#: column: the card story is generated by the card programme machinery rather
#: than by the episode overlay. Its narrowing is the same shape — the deep
#: sub-bucket the story is about, then the behavioural corroboration, then the
#: origination bands — but read off its own columns.
ALPHA_NARROWING = {
    "S1": ("dpd_20_29", "20-29 days past due"),
    "S2": ("dpd_20_29", "20-29 days past due"),
    "S3": ("high_utilisation", "utilisation at or above 90%"),
    "S4": ("high_utilisation", "utilisation at or above 90%"),
    "S5": ("high_utilisation", "utilisation at or above 90%"),
}


def _alpha_narrow(data: pd.DataFrame, rule: str) -> np.ndarray:
    if rule == "dpd_20_29":
        dpd = pd.to_numeric(data.get("dpd"), errors="coerce")
        return ((dpd >= 20) & (dpd <= 29)).fillna(False).to_numpy()
    if rule == "high_utilisation":
        dpd = pd.to_numeric(data.get("dpd"), errors="coerce")
        util = pd.to_numeric(data.get("utilisation_ratio"), errors="coerce")
        return (((dpd >= 20) & (dpd <= 29))
                & (util >= 0.90)).fillna(False).to_numpy()
...
def cohort_mask(data: pd.DataFrame, case_id: str, step: str) -> np.ndarray:
    ...
    if case_id == "C01":
        rule = ALPHA_NARROWING.get(step)
        if rule:
            mask = mask & _alpha_narrow(data, rule[0])
    ...
    if level:
        narrowed = mask & em.evidence_mask(data, level)
        # A population that carries no evidence state at all would be emptied
        # by this, which is not "nobody survived verification" — it is "this
        # book does not record verification for these rows". Said in the log
        # and left un-narrowed rather than reported as an empty cohort.
        if mask.any() and not narrowed.any():
            logger.warning(
                "%s carries no episode evidence state, so %s could not be "
                "narrowed by verification", case_id, step)
        else:
            mask = narrowed
    if step in STEP_IN_POCKET:
        mask = mask & em.pocket_mask(data, case_id, eligible)
...
def cohort_scope(case_id: str, step: str) -> dict[str, Any]:
    ...
    if case_id == "C01":
        rule = ALPHA_NARROWING.get(step)
        if rule:
            clauses.append(ch.Predicate("dpd", "in", "20-29", rule[1]))
    else:
        level = STEP_EVIDENCE.get(step)
        if level:
