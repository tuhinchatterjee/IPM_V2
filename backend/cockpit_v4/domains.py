...
def _configured(domain_id: str, published: str) -> str:
    """The release this DEPLOYMENT points this book at.

    A deployment that holds a different book of the same kind -- a bank's own
    retail portfolio rather than the demonstration one -- names it here, and
    every reader of `DEFAULT_RELEASES` follows: `domain_resolver.scope_for`,
    `availability`, `analytical_runtime.domain_of_release` and the seeder.
    Unset, which is the normal case, this is exactly the constant above and
    nothing about the runtime changes.

    It is a release id and nothing else. It cannot add a domain, widen a
    scope, name another book's relations or reach outside the lake: an id
    that is not published simply makes that book unaskable, said plainly, by
    the mechanism that already reports a missing release.
    """
    return os.environ.get(
        f"COCKPIT_V4_{domain_id.upper()}_RELEASE_ID", "").strip() or published


DEFAULT_RELEASES = {domain_id: _configured(domain_id, published)
                    for domain_id, published in DEFAULT_RELEASES.items()}
