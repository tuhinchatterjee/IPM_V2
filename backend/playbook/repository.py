...
def add_source(session, workspace_id: int, scope: Scope, *, filename: str,
               ...
               source_role: str = "supporting") -> PlaybookSource:
...
def set_chunks(session, source: PlaybookSource, chunks: list) -> None:
    """Replace a source's parsed content. Re-parsing is idempotent."""
    session.query(PlaybookSourceChunk).filter(
        PlaybookSourceChunk.source_id == source.id
    ).delete(synchronize_session=False)
    for chunk in chunks:
        row = chunk.as_row()
        session.add(PlaybookSourceChunk(
...
def sources(session, workspace_id: int) -> list[PlaybookSource]:
    stmt = (select(PlaybookSource)
            .where(PlaybookSource.workspace_id == workspace_id)
            .order_by(PlaybookSource.id))
    return list(session.execute(stmt).scalars())


def chunks(session, source_id: int) -> list[PlaybookSourceChunk]:
    stmt = (select(PlaybookSourceChunk)
            .where(PlaybookSourceChunk.source_id == source_id)
