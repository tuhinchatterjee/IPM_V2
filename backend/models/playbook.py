...
# ---------------------------------------------------------------------------
# The chat-first workspace's tables.
#
# Both subsystems arrived with a table called `playbook_sources`, meaning
# different things: a committee pack's CITED SOURCE above, and an UPLOADED
# DOCUMENT BEING READ below. One name, two meanings, and a foreign key that
# would have pointed at whichever migration ran last.
#
# The workspace's two are therefore `playbook_workspace_sources` and
# `playbook_workspace_source_chunks`, with `PlaybookWorkspaceSource` and
# `PlaybookWorkspaceSourceChunk` as their classes. The committee's keeps the
# original name because it is the system of record and its table already
# carries pack history.
# ---------------------------------------------------------------------------
...
class PlaybookWorkspaceSource(Base):
    ...
    __tablename__ = "playbook_workspace_sources"
    ...
    chunks: Mapped[list[PlaybookWorkspaceSourceChunk]] = relationship(
...
class PlaybookWorkspaceSourceChunk(Base):
    ...
    __tablename__ = "playbook_workspace_source_chunks"

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    source_id: Mapped[int] = mapped_column(
        ForeignKey("playbook_workspace_sources.id", ondelete="CASCADE"), nullable=False
    )
    ...
    source: Mapped[PlaybookWorkspaceSource] = relationship(back_populates="chunks")
...
class PlaybookAttachment(Base):
    ...
    source_id: Mapped[int | None] = mapped_column(
        ForeignKey("playbook_workspace_sources.id", ondelete="CASCADE"), nullable=True
    )
