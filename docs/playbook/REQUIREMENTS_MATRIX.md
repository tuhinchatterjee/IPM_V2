...
**No row is BLOCKED any longer.** The six criteria that needed a real provider
— PB-013, PB-015, PB-017, PB-029, PB-030 and PB-043 — have been exercised live
against `claude-opus-5` with recorded request ids, on local runs against this
branch. Those rows carry their own served model, timing and request id below;
none of them is written up from a scripted provider.

The distinction that produced those rows is kept, because it is what made the
live failures findable: a scripted-provider test is never reported as live
verification, and `scripts/playbook_live_slice.py` still exits 2 rather than 0
when no credential is present, so a run that did not happen can never read as a
pass.
...
| PB-013 | A supplied methodology can be checked against a prior report with a sourced coverage matrix and no unauthorized edit. | prompts.COVERAGE_CHECK wired through service._framed; task="coverage" on the message API | test_service.py asserts the framing sent forbids writing or revising the report; seeded IFRS 9 thread. **Live PASS: the `coverage_matrix` check produced 2 tables and left the report it checked unedited — `claude-opus-5`, check 52.3s (authoring 51.6s, rendering 0.1s), request_id `req_011Cf11d9qC1ChjDNvYfq89c`.** | PASS |
...
| PB-015 | A detailed no-template report can be generated from selected evidence; missing tests are not invented. | prompts.CREATE_WITHOUT_TEMPLATE via service._framed; authoring and rendering split — the provider call is text-only and `render.render()` produces the files deterministically | **Live PASS on the pushed branch: a complete 16-section report from evidence alone, inventing no test — `claude-opus-5`, 1 turn, 0 tool calls, Skills off, authoring 86.1s, local rendering 3.7s, check 90.4s, request_id `req_011Cf11GfGASgT27yejEhJVm`. Saved report grounded: yes. Unsupported financial figures in the saved report: 0. Unsupported tests: 0. Required supported figure 22.77 retained.** Reached after two earlier live failures, both fixed here: a 281s read timeout (one call was authoring AND driving the document Skills — authoring is now text-only by default and `render.render()` produces every format locally), and a `grounding FAILED` that was two defects — the check asserted the state of the model's FIRST DRAFT rather than of the saved report, and `figures()` demanded evidence for every digit including `CET1`, `IFRS 9`, `v2.1`, `see section 10.2`, `30 June` and `Q2 2026`. Authoring now runs a two-stage gate (repair, then verify what repair produced; nothing is persisted unless the second pass is clean) and `validate.classify` names each number by syntactic position, never by value. 42 tests in `test_grounding_gate.py`, 26 in `test_structural_numerals.py`. | PASS |
...
| PB-017 | Directly requested editorial edits work without a redundant approval loop and preserve facts/risk meaning. | `backend/playbook/merge.py` scoped_merge — only the requested section is taken from the model, every other section is carried forward from the approved version as the same object; prompts.SCOPED_EDIT; add_current_version | **Live PASS on `ca1d24a`: the executive summary was edited and nothing else — unrelated model rewrites discarded by the deterministic scoped merge, V1 preserved, V2 written, supported figures preserved, 39 figures traced. `claude-opus-5`, check 136.6s (authoring 52.9s, local rendering 0.2s).** Reached after a live failure that reported 13 unrelated sections changed with near-identical before/after text. Two causes, both confirmed by execution: the merge base was a Markdown re-render of the approved version, which drops every citation locator, `subtitle` and `meta` and injected a phantom section named after the title — so the damage really was written to canonical storage; and `_fingerprint` compared `repr(block.data)`, which is sensitive to dict key order that JSONB does not preserve, so every section holding a table reported as changed after a storage round-trip with nothing altered. The base is now the stored canonical `Document`, grounding takes a scope, and the live check compares stored canonical against stored canonical while printing canonical and rendered hashes, the first differing field and character, and a canonical-drift vs render-normalisation verdict. 19 tests in `test_scoped_edit_fidelity.py` plus 16 in `test_merge.py`. | PASS |
...
| PB-029 | Continuing a seeded thread invokes the real provider when configured; it is not scripted fixture playback. | service.send_message on a seeded workspace | live check `seeded_continuation` asserts the reply is `assistant_live` with real request ids, on a question no fixture contains. **Live PASS: origin=assistant_live, requests=1, job=1, `claude-opus-5`, check 17.5s, request_id `req_011Cf11gwHzRuCLQYt4V7Vt5`.** | PASS |
| PB-030 | Runtime actually uses the configured Opus model and tested artifact tools, with no hidden model downgrade. | backend/llm/roles.py AUTHOR; provider.AuthoringResult.downgraded; provider.status() and author() refuse with AUTHOR_MODEL_NOT_CONFIGURED rather than calling the SDK without a model | **Live PASS, confirmed on every run on this branch: requested `claude-opus-5`, served `claude-opus-5`, no silent downgrade.** Recorded request ids include `req_011CetFik5iB1janNWebTPLA` (76.5s) and the final suite's `req_011Cf11GfGASgT27yejEhJVm`, `req_011Cf11d9qC1ChjDNvYfq89c`, `req_011Cf11gwHzRuCLQYt4V7Vt5` and `req_011Cf11iCUKv9UUfxkinUmSe`. Plus test_provider_bounds.py model-guard tests and test_escalation.py role config. | PASS |
...
| PB-042 | Available-module regression and repository lint/type/build checks pass or have exact baseline-supported classifications. | ruff, pytest, tsc, eslint, next build, npm test | full repository suite 10,528 passed / 30 skipped / 0 failed; see docs/playbook/UAT_REPORT.md | PASS |
| PB-043 | Fresh prompts not present in seeds succeed through the live path; mocked tests are reported separately. | fresh prompts through the live path; `backend/validation/live_playbook.py`, driven by tests/playbook/test_live_playbook.py and scripts/playbook_live_slice.py | live check `fresh_prompt` asserts the answer is not a substring of any seeded turn. **Live PASS: a question absent from every fixture was genuinely answered by the real model — `claude-opus-5`, check 20.1s (authoring 19.5s, rendering 0.1s), request_id `req_011Cf11iCUKv9UUfxkinUmSe`.** 8 structural tests assert the suite covers every live requirement and refuses to run without a credential, so mocked results can never be reported as live ones. | PASS |
...
| PB-045 | Final report provides actual commit, test evidence, unresolved issues, runnable UAT instructions, and truthful readiness labels. | docs/playbook/UAT_REPORT.md; PROGRESS.md | this matrix, `docs/playbook/UAT_REPORT.md` and `PROGRESS.md`, with every live row carrying its served model, timing and request id, and scripted-provider evidence reported separately from it | PASS |
...
| BLOCKED | 0 |
| SKIPPED | 0 |
| DEFERRED-INTEGRATION | 0 requirements — one module, below |

All 45 requirements pass. Six of them pass on live `claude-opus-5` runs with
recorded request ids rather than on a scripted provider: PB-013, PB-015,
PB-017, PB-029, PB-030 and PB-043.

No requirement is DEFERRED-INTEGRATION. **One module is** — What If, which has
no producing surface on this baseline and so has no row of its own. It is
recorded against PB-006 and PB-044 and in `docs/playbook/INTEGRATION_NOTES.md`.
