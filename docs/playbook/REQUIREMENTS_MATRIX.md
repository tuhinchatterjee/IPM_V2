...
| PB-013 | A supplied methodology can be checked against a prior report with a sourced coverage matrix and no unauthorized edit. | prompts.COVERAGE_CHECK wired through service._framed; task="coverage" on the message API | test_service.py asserts the framing sent forbids writing or revising the report; seeded IFRS 9 thread. Running the check needs a provider. | BLOCKED |
...
| PB-015 | A detailed no-template report can be generated from selected evidence; missing tests are not invented. | prompts.CREATE_WITHOUT_TEMPLATE wired through service._framed; task="create" | test_service.py asserts the framing names the document family and forbids describing an absent test as performed; pipeline tested with a scripted author. Generating one needs a provider. | BLOCKED |
...
| PB-017 | Directly requested editorial edits work without a redundant approval loop and preserve facts/risk meaning. | prompts.SCOPED_EDIT wired through service._framed; the current document travels with every revision; task/scope on the message API; nextSteps chips carry their framing | test_service.py: 7 framing tests incl. the scoped edit naming its scope, forbidding other changes and refusing to soften a negative finding, and a revision being shown every section of the document it is revising; test_api.py task/scope; playbook.test.ts chip framings. Performing the edit needs a provider. | BLOCKED |
...
| PB-038 | Streaming, cancellation, retry, refresh, and duplicate submission do not corrupt state or create duplicate final versions. | playbook_jobs.idempotency_key unique; service.request_cancel and cancellation_watcher; GET /jobs/by-key/{key}, GET /jobs/{id}, POST /jobs/{id}/cancel | test_messages.py duplicate-send and 5 stop tests; test_api.py job lookup and cancel. **Token streaming is not implemented**: generation is synchronous and reports real milestones, never a percentage. | PASS |
...
| PB-040 | Every visible control is exercised; keyboard/focus behavior and laptop layout are verified. | scripts/acceptance/playbook_browser_acceptance.py | 90 checks at 2 viewports; Escape/focus; the change panel, the restore control, the preview and source correction driven end to end; no console errors | PASS |
...
| PB-042 | Available-module regression and repository lint/type/build checks pass or have exact baseline-supported classifications. | ruff, pytest, tsc, eslint, next build, npm test | full backend suite 9753 passed / 22 skipped / 0 failed; see docs/playbook/UAT_REPORT.md | PASS |
