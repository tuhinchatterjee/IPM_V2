...
| PB-015 | A detailed no-template report can be generated from selected evidence; missing tests are not invented. | prompts.CREATE_WITHOUT_TEMPLATE wired through service._framed; task="create" | test_service.py asserts the framing names the document family and forbids describing an absent test as performed; pipeline tested with a scripted author. The live check `no_template_report` asserts ≥4 sections, grounding held, both files produced, and that no absent backtest is described as performed. **Running it needs a provider: BLOCKED.** | BLOCKED |
...
| PB-017 | Directly requested editorial edits work without a redundant approval loop and preserve facts/risk meaning. | prompts.SCOPED_EDIT wired through service._framed; the current document travels with every revision; task/scope on the message API; nextSteps chips carry their framing | test_service.py: 7 framing tests incl. the scoped edit naming its scope, forbidding other changes and refusing to soften a negative finding, and a revision being shown every section of the document it is revising; test_api.py task/scope; playbook.test.ts chip framings. The live check `scoped_edit` asserts v2 is written, v1 is byte-identical afterwards, and every two-decimal figure in v1 survives into v2. **Running it needs a provider: BLOCKED.** | BLOCKED |
...
| PB-042 | Available-module regression and repository lint/type/build checks pass or have exact baseline-supported classifications. | ruff, pytest, tsc, eslint, next build, npm test | full backend suite 9802 passed / 22 skipped / 0 failed; see docs/playbook/UAT_REPORT.md | PASS |
