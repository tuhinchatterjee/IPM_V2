...
| `PLAYBOOK_MAX_TURNS`, `PLAYBOOK_MAX_OUTPUT_TOKENS` | Bounds on one authoring run. The output ceiling defaults to 64,000 — the documented default for a STREAMED request, which is what this is. It was 16,000, the non-streaming default, and a report that reached it was delivered looking finished because `stop_reason` was read by nothing. A turn that hits the ceiling now fails and says so; nothing truncated is saved, and it is not retried, because the next attempt would spend the same tokens to reach the same ceiling. |
| `PLAYBOOK_SHOW_REASONING` | On by default. Asks the provider for a summarised account of what the model is working on and streams it on its own channel while the turn runs. On this model reasoning is on regardless and its display defaults to omitted, which is why the opening minutes of a run looked like a hang. It is shown, never stored, and never merged into the answer or the document; if the provider refuses the parameter it is dropped for the process and the run continues. |
| `PLAYBOOK_WRITTEN_ASSESSMENT` | On by default. After a turn that wrote a version, one bounded 700-token call reads the completion card — counted from stored rows — and writes a short note on it. It is given the card and nothing else, it is labelled as judgement wherever it is shown, and turning it off leaves the counted card exactly as it was. |
