...
Fifteen minutes, four screens, no slides. Everything shown is calculated live
...
## 5. Create a lens — 4 minutes

Click **Create a lens**.

**Step 1 — the name.** Type `Retail Collections Watch`. Press Continue.

> It asked what to call it first, because the name is what it matches against
> to suggest the rest. Nothing here is generated by a model: the name is
> matched against the Metric Catalogue with the same search the typeahead
> uses, so the same name suggests the same thing on every machine, and it
> cannot suggest a domain you are not allowed to read.

**Step 2 — what it is for.** The definition panel: purpose, audience,
portfolio, data domains, the period it opens on, what it compares against,
visibility.

Fill the purpose. Pick **Retail Credit Risk** as the domain. Click **Use this**.

> Seven fields, not twenty. And it is asked before the metrics on purpose — a
> lens whose scope is decided after its tiles is a lens whose tiles decided its
> scope, and it ends up being about whatever was easy to find.

**Step 3 — what it shows.** Type `del` in the search.

> It did not open with the catalogue. Seventy-seven metrics in a scrolling list
> is a list nobody reads, and people rebuild a number they already had — which
> is how two definitions of "default rate" end up on two dashboards.

Point at a row.

> Every suggestion says its unit, its definition, and the formula — because
> "30+ DPD" by count and by balance have almost the same name and are
> different numbers. And it says *why* it matched: a suggestion whose reason is
> invisible looks like a bug.

Now type `delinq 30`.

> The list got shorter, not longer. Every word you add can only remove
> candidates.

Now type `bad rate`.

> It reaches the default rate. "NPL rate", "bad rate" and "default rate" are
> three names people use for one number, and a catalogue that only answers to
> the first is a catalogue people stop searching.

Add two or three metrics. Click **Create the lens**.
...
- **Natural-language transformation plans.** The safe transformation IR exists
  and the metric builder uses it. Describing the logic in a sentence and having
  CreditProbe turn it into an editable deterministic plan is not built.
...
three are reinstalled by the command above and keep their revision history.
