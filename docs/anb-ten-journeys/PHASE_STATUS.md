...
| P10 | Real-browser, data and regression       | IN PROGRESS (journeys and X-checks done; full-suite attribution running) |
| P11 | Freeze and handoff                      | IN PROGRESS (launcher verified; manifests pending the regression) |
...
## P10 — Real-browser, data and regression verification

### Ten journeys in a real browser

`scripts/cpra_browser_journeys.py` drives Chromium against the running build on
5334/8334: Cockpit, card, drawer, Investigate, the five chips in order, the
...
Every figure was already checked against the book. What the gates could not
check is whether a reader can GET to it.

1. **The case seed carried the entity's label, not its identifier.** A thread
   opened from one of the ten knew what KIND of finding it was and not WHICH
   one, so the five prompts could not be built and every one of them fell
   through to the planner. No error anywhere; the page simply rendered without
   them.
2. **The opening question threw the card away.** An episode investigation asked
   "something seems wrong with DEMO Employer A / private facilities services /
   Eastern Province" — the generic segment question, discarding the rate, the
   comparator and the pocket the card had just established.
3. **The decomposition read a column the book does not carry.**
   `pd.to_numeric(None)` returns a scalar rather than a Series, so Alpha's
   workbook raised 500 instead of naming the missing driver.

Two of my own assertions were also wrong rather than the pages: chips in a
sticky footer are not in `inner_text`, and a false failure costs as much here
as a false pass.

Alpha now carries the same five chips as the other nine. Its card is raised by
the accepted early-delinquency rule and says so, but the five questions on that
thread are its own.

### The thirty cross-cutting checks

`tests/retail/test_ret_cpra_p10_crosscutting.py`: **26 gates, all passing**,
covering X01 and X03-X29. X02 and X30 are the launcher's start-stop-start,
recorded below. Several are negative controls written to fail loudly if the
thing they guard starts happening — a temporal-leakage check that reads the
months a step actually touched, a hostile note that tries to grant itself
compliance, a second reader refused on both the cohort and the investigation.
...
**90 pass, 0 fail, 0 not run.** The fixture's expected counts sit beside this
book's measured ones rather than being asserted against them, which is the
specification's own instruction: a book of 59,416 facilities computes its own
counts.
...
`launchers/anb2/start-anb2.command` and `stop-anb2.command`, on 5334/8334.

Four builds of this repository now exist side by side and three of them are
somebody's presentation. The launcher checks its configured ports against a
written-down list of all three protected pairs, so an edit to the two at the
top cannot quietly make the check pass. It stops nothing it did not start: a
port held by somebody else is a refusal that names the holder and says
explicitly that nothing was stopped.

The database is read from `.env.anb2` rather than written into the launcher,
and a connection string it recognises as the accepted demonstration's is
refused — rebuilding the data, migrating it or replaying the review all write
there.
...
work opened, changed or inspected anything on the Mac, and nothing claims to
have.

### Demonstration login

Seeded through the product's own CLI:

```
.venv/bin/python scripts/manage_users.py add anb.analyst --role admin
```

The password is chosen at setup time and is not recorded here or anywhere in
the repository.
