...
```
PASS   16 journeys, 80 checks, 80 passed, 0 failed, in 350s   (three consecutive runs)
...
four times before the number above meant anything, and each fix is worth
stating because each was producing a confident wrong answer:

* **The first version compared the rendered text and the URL.** Five prompt
  buttons — on What-If, Lenses and the Playbook — write the question into the
  composer, which is neither. All five were reported dead. A form value is now
  part of what counts.
* **It walked the page's buttons by position while its own clicks moved
  them.** The notifications bell opens a panel of 1,600 characters; reached a
  second time with the panel already open, pressing it closes the panel and
  lands back on the exact fingerprint it started from. It was reported dead on
  four screens. A dead verdict is now only ever recorded after the control has
  been pressed again from a CLEAN load, found by its own name.
* **It could not see a control that was already selected.** "All" on the
  Cockpit and "Highest PD" on Borrower 360 do nothing when pressed because
  they are already applied, and most such controls say so only in a CSS class.
  A would-be-dead control is now offered a sibling in its own group and pressed
  again; one that answers that is selected, not dead.
* **It clicked "Collapse navigation" on the first screen and audited the other
  ten in a collapsed shell**, where the bell is unreachable — a fifteen-second
  timeout on a control that works. The shell's own buttons are now pressed at
  the END, each from a fresh load of the screen it appears on, so nothing a
  shell control does is inherited by a page audit.

A Refresh button is also no longer dead by definition: it re-reads
`GET /whatif/landing`, the figures come back the same because nothing has
changed, and the page is character-for-character what it was. **API calls made
during the click** are part of the fingerprint.

One more, found by running the suite twice and getting two different answers:
a control that lives inside a panel the audit itself opened is not on the page
after a clean reload, and waiting fifteen seconds for a hidden element to
become clickable was being reported as a broken control. Recovery now dismisses
whatever is on top and retries WHERE THE CONTROL STANDS first; the clean reload
is the second resort, and a control a fresh load does not show is counted as
needing a panel rather than failed.
...
| 8 | **The Cockpit's opening screen fell to one fallback question.** `opening()` chose among the five approved questions by reading `context.datasets` — a governed context built for an EMPTY question, where every dataset scores zero and which eight come back is settled by the tie-breakers, in effect alphabetically. Registering three Early Warning datasets pushed `portfolio_facility` out of the eight, no approved question could be answered, and the screen offered "What data do you have in Early Warning — Borrower Month?" instead of three governed questions. The datasets a suggestion needs are now asked for BY NAME | the broad backend regression, `tests/api/test_cockpit_suggestions.py` |
| 9 | **`.env.example` shipped `AI_COCKPIT_PREPROCESS_MODEL=<explicit-sonnet-model>` and `AI_COCKPIT_REASONING_MODEL=<explicit-opus-model>`.** A placeholder naming a model family is not a neutral stand-in: `/api/v1/ask/mode` reads it back and reports it as the configured id, so the deployment named a vendor's model as its own choice while being unable to call it. Blank fails closed with `MODEL_CONFIGURATION_MISSING`, which is the honest state. **19 of the 418 non-passing tests were this one line** | the broad regression, `tests/release/test_product_copy.py` and `tests/validation/` |
| 10 | **The Lens boundary registry did not name the four datasets this pass registered**, so whether a Lens could read them was being decided by a Data Builder domain label rather than by the registry that exists to decide it | `tests/metrics/test_lens_domains.py` |
