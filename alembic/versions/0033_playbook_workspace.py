...
def upgrade() -> None:
    ...
    op.create_table('playbook_sources',
    sa.Column('id', sa.BigInteger(), nullable=False),
    sa.Column('workspace_id', sa.BigInteger(), nullable=False),
    sa.Column('tenant', sa.String(length=64), nullable=False),
    ...
    op.create_index('ix_playbook_sources_workspace', 'playbook_sources', ['workspace_id', 'created_at'], unique=False)
    ...
    op.create_table('playbook_source_chunks',
    sa.Column('id', sa.BigInteger(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['source_id'], ['playbook_sources.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('source_id', 'ordinal', name='uq_playbook_source_chunk')
    )
    op.create_index('ix_playbook_source_chunks_source', 'playbook_source_chunks', ['source_id', 'ordinal'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index("ix_playbook_source_chunks_source", table_name="playbook_source_chunks")
    op.drop_table("playbook_source_chunks")
    ...
    op.drop_index("ix_playbook_sources_workspace", table_name="playbook_sources")
